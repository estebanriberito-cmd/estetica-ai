...
export default function Metricas() {
  ...
  const RESUMEN = [
    ...
    { label: "Reagendados",   val: reagendados,             color: "#c9a0ff", Icon: RescheduleIcon },
    { label: "Cancelados",    val: cancelados,              color: "#f07070", Icon: CloseIcon      },
    { label: "Por Instagram", val: porCanal.Instagram || 0, color: "#D926FF", Icon: InstagramIcon  },
    { label: "Por WhatsApp",  val: porCanal.WhatsApp  || 0, color: "#1D9E75", Icon: MessageIcon    },
  ]

  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: "var(--bg)", overflowY: "auto" }}>
      <div style={{
        flexShrink: 0, padding: "16px 22px", borderBottom: "1px solid var(--border)",
        ...
        <div style={{ display: "flex", gap: 4 }}>
          {[["hoy", "Hoy"], ["semana", "Esta semana"], ["mes", "Este mes"]].map(([val, label]) => (
            <button key={val} onClick={() => setPeriodo(val)} style={{
              fontSize: 11, fontWeight: periodo === val ? 500 : 400, padding: "5px 12px", borderRadius: "var(--radius-sm)",
        ...
        <div style={{ padding: "18px 22px", display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
            {KPIS.map(({ label, val, color, bg, border }) => (
              <div key={label} style={{ background: bg, border: `1px solid ${border}`, borderRadius: "var(--radius)", padding: "14px 16px" }}>
                <div style={{ fontSize: 10, color: "var(--text-3)", marginBottom: 8, fontWeight: 500 }}>{label}</div>
                <div style={{ fontSize: 26, fontWeight: 600, color, letterSpacing: "-0.5px" }}>{val}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              display: "flex", alignItems: "center", justifyContent: "center", color: "#c9a0ff",
            }}>
              <AIIcon />
            </div>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: "var(--text-3)" }}>{conversion}% de automatización en este período</div>
            </div>
            <div style={{ fontSize: 26, fontWeight: 700, color: "#1D9E75", letterSpacing: "-0.5px" }}>{conversion}%</div>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div style={{ background: "var(--surface-2)", border: "1px solid var(--border-2)", borderRadius: "var(--radius)", padding: "14px 16px" }}>
              ...
              <div style={{ display: "flex", alignItems: "flex-end", gap: 5, height: 90, position: "relative" }}>
                {[0.25, 0.5, 0.75, 1].map(pct => (
                  <div key={pct} style={{ position: "absolute", left: 0, right: 0, bottom: `${pct * 72}px`, height: 1, background: "rgba(255,255,255,0.04)", pointerEvents: "none" }} />
                ))}
                {diasKeys.map(dia => {
                  ...
                  return (
                    <div key={dia} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
                      <span style={{ fontSize: 9, color: hasVal ? "#c9a0ff" : "transparent", fontWeight: 500 }}>{val}</span>
                      <div style={{ width: "100%", height: h, borderRadius: hasVal ? "3px 3px 0 0" : "2px", background: hasVal ? "linear-gradient(to top, #7B2FFF, #b060ff)" : "rgba(255,255,255,0.04)" }} />

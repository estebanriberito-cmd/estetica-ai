...
const ACTIONS = [
  { label: "Confirmar", color: "#1D9E75", bg: "rgba(29,158,117,0.10)",  border: "rgba(29,158,117,0.22)",  Icon: CheckIcon  },
  { label: "Reagendar", color: "#c9a0ff", bg: "rgba(123,47,255,0.12)",  border: "rgba(123,47,255,0.28)",  Icon: ClockIcon  },
  { label: "Cancelar",  color: "#f07070", bg: "rgba(240,112,112,0.10)", border: "rgba(240,112,112,0.22)", Icon: CloseIcon  },
  { label: "Perdido",   color: "#3a3a3a", bg: "rgba(255,255,255,0.03)", border: "#1e1e1e",                Icon: GhostIcon  },
]
      ...
      <div style={{
        ...
        <div style={{ padding: "16px 14px 12px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
            ...
            <span style={{
              ...
              background: "var(--primary-10)", color: "#c9a0ff",
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          {loading ? (
            ...
            const active = activo?.id === t.id
            const estadoStyle = ESTADO[t.estado] || ESTADO.confirmado
            const canalStyle  = CANAL[t.canal]  || CANAL.Instagram
            return (
              <div key={t.id} onClick={() => setActivo(t)}
                style={{
                  display: "flex", gap: 10, padding: "12px 14px",
                  borderBottom: "1px solid #161616",
                  borderLeft: active ? "2px solid #7B2FFF" : "2px solid transparent",
                  background: active ? "rgba(123,47,255,0.07)" : "transparent",
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 3 }}>
                    <span style={{ fontSize: 12, fontWeight: 500, color: active ? "#e8e8e8" : "#bbb" }}>
                      {t.nombre || "Sin nombre"}
                    </span>
                    <span style={{ fontSize: 9, color: "var(--text-4)", flexShrink: 0, marginLeft: 6 }}>
                      {t.hora_turno || ""}
                    </span>
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 5 }}>
                    <Badge label={t.canal || "Instagram"} {...canalStyle} />
                    <Badge label={t.estado || "confirmado"} {...estadoStyle} />
                  </div>
                </div>
              </div>
            )
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        {activo ? (
          <>
            <div style={{
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 10, color: "var(--text-3)", marginTop: 2 }}>{activo.canal || "Instagram"} · {activo.servicio}</div>
              </div>
              <button style={{
                display: "flex", alignItems: "center", gap: 6,
                padding: "7px 14px",
                background: "var(--primary-10)", border: "1px solid var(--primary-25)",
                borderRadius: "var(--radius-sm)", fontSize: 11, fontWeight: 500, color: "#c9a0ff",
                transition: "all 0.15s",
              }}
                onMouseEnter={e => { e.currentTarget.style.background = "var(--primary-15)"; e.currentTarget.style.borderColor = "rgba(123,47,255,0.45)" }}
                onMouseLeave={e => { e.currentTarget.style.background = "var(--primary-10)"; e.currentTarget.style.borderColor = "var(--primary-25)" }}
              >
                <HandIcon /> Tomar conversación
              </button>
            </div>
              ...
              {mensajes.map((m, i) => {
                ...
                return (
                  <div key={m.id} style={{
                    ...
                    marginTop: first ? (i === 0 ? 0 : 12) : 3,
                  }}>
                    {m.tipo === "bot" && first && (
                      <div style={{ marginBottom: 4 }}>
                        <span style={{
                          fontSize: 9, fontWeight: 600, padding: "1px 6px", borderRadius: 20,
                          background: "var(--primary-10)", color: "#7B2FFF",
                          border: "1px solid rgba(123,47,255,0.22)", letterSpacing: "0.4px",
                        }}>AI</span>
                      </div>
                    )}
                    <div style={{
                      padding: "8px 13px", borderRadius: bubbleRadius(m.tipo, first, last),
                      background: m.tipo === "user" ? "rgba(123,47,255,0.16)" : "var(--surface-2)",
                      color: m.tipo === "user" ? "#ddd0ff" : "#999",
                      border: `1px solid ${m.tipo === "user" ? "rgba(123,47,255,0.28)" : "var(--border-2)"}`,
                      fontSize: 12, lineHeight: 1.55,
                    }}>
                      {m.texto}
                    </div>
                    {last && (
                      <div style={{ fontSize: 9, color: "var(--text-4)", marginTop: 4, textAlign: m.tipo === "user" ? "right" : "left" }}>
                        {m.hora}
                      </div>
                    )}
                  </div>
            ...
            <div style={{ flexShrink: 0, display: "flex", gap: 5, padding: "9px 16px", borderTop: "1px solid var(--border)", background: "var(--bg)" }}>
              {ACTIONS.map(({ label, color, bg, border, Icon }) => (
                <button key={label} style={{
                  display: "flex", alignItems: "center", gap: 5, padding: "6px 12px",
                  borderRadius: "var(--radius-sm)", fontSize: 11, fontWeight: 500,
                  background: bg, border: `1px solid ${border}`, color, transition: "opacity 0.15s",
                }}
            ...
            <div style={{ flexShrink: 0, padding: "9px 16px 14px", display: "flex", alignItems: "center", gap: 8, background: "var(--bg)" }}>
              <div style={{
                ...
                borderRadius: "var(--radius)", padding: "9px 14px", fontSize: 12, color: "#2a2a2a",
              }}>
                Escribir mensaje...
              </div>
              <button style={{
                ...
                display: "flex", alignItems: "center", justifyContent: "center",
                color: "#fff", boxShadow: "0 2px 10px rgba(123,47,255,0.38)", transition: "background 0.15s, box-shadow 0.15s",
              }}
                onMouseEnter={e => { e.currentTarget.style.background = "#9044ff"; e.currentTarget.style.boxShadow = "0 2px 16px rgba(123,47,255,0.55)" }}
                onMouseLeave={e => { e.currentTarget.style.background = "var(--primary)"; e.currentTarget.style.boxShadow = "0 2px 10px rgba(123,47,255,0.38)" }}
              >
                <SendIcon />
              </button>
            </div>
          ...
          <div style={{
            ...
            alignItems: "center", justifyContent: "center", gap: 8, color: "var(--text-4)",
